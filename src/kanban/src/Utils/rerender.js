...
export const noTaskMessage = new NoTaskMessage();

document.body.appendChild(header.element);
document.body.appendChild(main.element);
document.body.appendChild(footer.element);

export let firstNewTaskButton = document.body.getElementsByClassName(cssCard.itemMenuDown)[0];
export let restNewTaskButton = [...document.body.getElementsByClassName(cssCard.itemMenuDown)].slice(1);
...
export const rerender = () => {
    ...
    state.forEach((column, index) => {
        ...
        isPreviousTasks = (index === 0) ? true : Boolean(state[index - 1]?.issues.length);

        if (!isPreviousTasks) {
            card.element.children[2].firstElementChild.disabled = true;
        }
    
        if (column.issues.length) {
            isPreviousTasks = true;
...
export const newListButton = header.element.getElementsByClassName(cssHeader.addListButton)[0];
